...
export default function PenguinSunset() {
  ...
  useEffect(() => {
    ...
    // We add T00:00:00 to ensure the local browser respects the exact date selected
    const start = new Date(`${startDate}T00:00:00`).getTime();
    const now = new Date().getTime();
    const diff = now - start;
    const days = Math.floor(diff / (1000 * 60 * 60 * 24));
    setDaysWaiting(Math.max(0, days)); 
  }, [startDate]);
  ...
  const fetchDate = async () => {
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('action_type', 'penguin_date')
      .single();
      
    if (data && data.count) {
      // Decode our "YYYYMMDD" integer back into a standard date string
      const dateStr = data.count.toString();
      if (dateStr.length === 8) {
  ...
  const handleSaveDate = async (e: React.FormEvent) => {
    ...
    // THE FIX: Convert "2024-05-20" into the integer 20240520
    // This safely fits inside your standard integer column without throwing an error!
    const dateInt = parseInt(tempDate.replace(/-/g, ''), 10);
    
    const { error } = await supabase
      .from('affection_counters')
      .upsert({ action_type: 'penguin_date', count: dateInt });
            ...
            <div className="bg-white dark:bg-[#1a1a2e] p-6 rounded-2xl w-full max-w-sm">
              ...
              <form onSubmit={handleSaveDate} className="flex flex-col gap-4">
                <input 
                  ...
                  className="w-full p-3 rounded-xl bg-gray-100 dark:bg-black/50 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-pink-500"
                />
